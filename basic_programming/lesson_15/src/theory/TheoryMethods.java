...
public class TheoryMethods {
     ...
     * консоль.
     * <p>
     * Синтаксис modifier returnType nameOfMethod (Parameter List) { // тело метода } Приведенный выше синтаксис
     * включает: modifier – определяет тип доступа для метода и возможность его использования. returnType – метод может
     * возвратить значение. nameOfMethod – указывает имя метода. Сигнатура метода включает имя метода и перечень
     * параметров. Parameter List – перечень параметров представлен типом, порядком и количеством параметров метода.
     * Данная опция задается произвольно, в методе может присутствовать нулевой параметр. method body – тело метода
     * определяет метод работы с командами.
     * <p>
     * Методы с возвращаемым знаечением типа void в Java не производят возврат каких-либо значений, лишь выполняет
     * какую-то логику
     * <p>
